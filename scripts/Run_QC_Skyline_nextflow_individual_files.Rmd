...
df_intensity <- read.csv(params$skyline_report_csv,
                         check.names = FALSE)
df_intensity <- df_intensity[df_intensity$File_Name != "",]

#' filtering out M+ isotopes that are a part of the Skyline reports
df_intensity <- filter(df_intensity, Isotope_Dist_Index == 0)

df_intensity$ionization_mode <- ifelse(grepl("-", df_intensity$Precursor_Adduct), "ESI-", "ESI+")



#' reading in the formula csv
#' this contains the formulas for each internal standard
#' need this since skyline has a specific way of reporting its formula for heavy labelled compounds
#' and this gets joined to the skyline report file
Molecule_Formula <- read.csv(params$molecular_formula)
df_intensity <- left_join(df_intensity, Molecule_Formula)
...
if (dim(df_intensity_filt)[1] == 0) {
  ...
  filename <- unique(df_intensity$File_Name)
  ...
  column_ionization <- unique(df_intensity$ionization_mode)
...
if (dim(df_intensity_filt)[1] == 0) {
  ...
  df_intensity %>%
  select(Molecule_Name, Formula, Explicit_Retention_Time,	`Chromatogram_Precursor_M/Z`,	
         Precursor_Adduct,	ionization_mode, Molecule_List_Name) %>% 
  kable("latex", col.names = c("Name", "Molecular Formula", "Expected RT", "Expected m/z",
                              "Adduct",	"Ionization", "Quench/Resuspension QC"), 
...
keep_mol <- result_tidyverse %>%
  ...
  filter(chr_time >= (unique(Start_Time)-0.05) & chr_time <= (unique(End_Time)+0.05)) %>% 
...
for (compound in compounds_to_plot_EICs) {
  ...
  y_integration <- result_tidyverse %>% filter(Molecule_Name == compound) %>% 
    filter(chr_time >= (chr_start-0.05) & chr_time <= (chr_end+0.05))
...
# Intensity
Individual peak Intensity normalized to D10-Leucine peak intensity if D10-Leucine peak was measured.
...
# Mass tolerance (ppm)
Output provided as part of Skyline report.
...
# Peak Shape Scores
**FWHM** = width of peak in retention time space at 50% peak height (Output provided as part of Skyline report.)
